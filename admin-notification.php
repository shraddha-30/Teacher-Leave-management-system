...
?>
    ...
    <li class="dropdown">
        <i class="ti-bell dropdown-toggle" data-toggle="dropdown">
            ...
            <?php 
                $isread=0;
                $sql = "SELECT tblleaves.id as lid,tblteacher.FirstName,tblteacher.LastName,tblteacher.tid,tblleaves.PostingDate from tblleaves join tblteacher on tblleaves.tid=tblteacher.id where tblleaves.IsRead=:isread";
                $query = $dbh -> prepare($sql);
                $query->bindParam(':isread',$isread,PDO::PARAM_STR);
                $query->execute();
                $results=$query->fetchAll(PDO::FETCH_OBJ);
                ...
                {               ?>  
                     ...
                     <a href="teacherLeave-details.php?leaveid=<?php echo htmlentities($result->lid);?>" class="notify-item">
                       ...
                       <p><b><?php echo htmlentities($result->FirstName." ".$result->LastName);?>
                                            <br />(<?php echo htmlentities($result->tid);?>)
